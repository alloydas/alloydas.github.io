...
PUB_STATS_PATH     = Path("_data/pub_stats.yml")
...
def write_pub_stats(works: list) -> None:
    """
    Write per-year publication counts to _data/pub_stats.yml.

    The "Publication Timeline" chart on the homepage reads this, so the bar
    chart stays in step with the bibliography instead of being hand-edited
    every time a paper lands.
    """
    counts = {}
    for w in works:
    ...
    lines = [
        "# ─────────────────────────────────────────────────────────────────",
        "# pub_stats.yml  —  auto-generated by scripts/fetch_publications.py",
        "# Drives the Publication Timeline chart in _pages/about.md",
        "# DO NOT EDIT MANUALLY — re-run the script or GitHub Action",
        "# ─────────────────────────────────────────────────────────────────",
        "years:",
    ]
    for year in sorted(counts):
        lines.append(f"  - year: {year}")
        lines.append(f"    count: {counts[year]}")
    lines.append(f"total: {sum(counts.values())}")

    PUB_STATS_PATH.parent.mkdir(parents=True, exist_ok=True)
    PUB_STATS_PATH.write_text("\n".join(lines) + "\n", encoding="utf-8")
    span = f"{min(counts)}–{max(counts)}" if counts else "none"
    print(f"\n\U0001F4CA Wrote {PUB_STATS_PATH} ({sum(counts.values())} papers, {span})")
